...
if __name__ == '__main__':
    ...
    # name = "train"
    test = []
    # json_file = "/media/syb310/ubuntudata/Code/ultralytics-main/dataset/"+name+".json"
    ana_txt_save_path = arg.save_path  # 保存的路径
    # ana_txt_save_path = "/media/syb310/ubuntudata/Code/ultralytics-main/dataset/labels/"+name
    ...
    # 这里需要根据自己的需要，更改写入图像相对路径的文件位置。
            ...
            test.append(ana_txt_name)
            f_txt = open(os.path.join(ana_txt_save_path, ana_txt_name), 'w')
            for ann in data['annotations']:
                box = convert((img_width, img_height), ann["bbox"])
                f_txt.write("%s %s %s %s %s %s" % (0, box[0], box[1], box[2], box[3],  id_map[ann["category_name"]]))
