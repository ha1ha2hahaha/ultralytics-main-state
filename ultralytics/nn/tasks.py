...
class PoseModel(DetectionModel):
    ...
    def __init__(self, cfg='yolov8n-pose.yaml', ch=3, nc=None, ns=None, data_kpt_shape=(None, None), verbose=True):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    depth, width, kpt_shape, ns = (d.get(x, 1.0) for x in ('depth_multiple', 'width_multiple', 'kpt_shape', 'ns'))
    if scales:
        scale = d.get('scale')
        if not scale:
