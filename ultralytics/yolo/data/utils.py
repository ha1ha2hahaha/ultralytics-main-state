...
def verify_image_label(args):
    ...
    im_file, lb_file, prefix, keypoint, state, num_cls, nkpt, ndim = args
    # Number (missing, found, empty, corrupt), message, segments, keypoints
    # nm, nf, ne, nc, msg, segments, keypoints = 0, 0, 0, 0, '', [], None
    nm, nf, ne, nc, msg, segments, keypoints, states = 0, 0, 0, 0, '', [], [], None
    try:
        # Verify images
        ...
        if keypoint:
            ...
            num = lb.shape[1]/57
            ntb = lb.reshape(int(num), -1)
            keypoints = ntb[:, -51:].reshape(-1, 17, 3)
            if ndim == 2:
        ...
        lb = ntb[:, :5]
        states = ntb[:, 5]
        return im_file, lb, shape, states, segments, keypoints, nm, nf, ne, nc, msg
