        ...
        if len(cls) > 0:
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    flag = False
                    name = {0: "Stand", 1: "Sit", 2: "Squat", 3: "Walk", 4: "Lie", 5: "Fall", 6: "Bend", 7: "Kneel"}
                    if state is not None:
                        if len(state.shape)==2:
                            # st = torch.argmax(state, axis=1)
                            # sta = state[idx]
                            state = torch.nn.functional.softmax(state, dim=1)
                            # print(state)
                            conf, sta = torch.max(state, axis=1)
                            s = sta[idx]

                            con = conf[idx]
                            s = s.cpu().numpy()
                            con = con.cpu().numpy()
                            label = f'{name[int(s[j])]}   {str(con[j])[:5]}'
                            annotator.box_label(box, label, color=color)
                        else:
                            s = state[idx]
                            annotator.box_label(box, name[int(s[j])], color=color)
                    else:
                        label = f'{c}' if labels else f'{c}{conf[j]:.1f}'
                        annotator.box_label(box, label, color=color)
